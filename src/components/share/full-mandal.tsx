...
export function FullMandal({
    ...
    if (mainIndex < 0 || mainIndex >= 8) {
      return (
        <div className="grid grid-cols-3 gap-0.5 sm:gap-1">
          {[0, 1, 2].map((row) =>
            [0, 1, 2].map((col) => (
              <div key={`${mainIndex}-${row}-${col}`} className="aspect-square w-[60px] sm:w-[80px] md:w-[100px]">
                {renderCell("")}
              </div>
            ))
          )}
  ...
  const renderCell = (text: string | null | undefined, isCenter: boolean = false) => {
    ...
    return (
      ...
      >
        <span
          className={cn(
            "text-[10px] sm:text-xs md:text-sm break-keep flex items-center justify-center leading-[1.2] whitespace-pre-line",
            "line-clamp-2 sm:line-clamp-3 md:line-clamp-4",
            isCenter && "font-bold text-[11px] sm:text-sm md:text-base",
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto p-2 sm:p-4 bg-slate-950">
      <div className="grid grid-cols-3 gap-2 sm:gap-4 w-full h-full place-items-center">
        {renderSubMandal(0)}
        {renderSubMandal(1)}
        {renderSubMandal(2)}
        {renderSubMandal(3)}
          ...
          {[0, 1, 2].map((row) =>
            [0, 1, 2].map((col) => {
              ...
              return (
                <div
                  key={`main-${row}-${col}`}
                  className="aspect-square w-[60px] sm:w-[80px] md:w-[100px]"
                >
